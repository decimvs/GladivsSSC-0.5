 ...
 * @author Guillem
 */
public class sscCircle implements sscShape {
    
    private Circle shape;        //Figura
    private Circle selection;   //Selecció
    ...
    protected double origCenterX, origCenterY, origRadius;
    ...
    public sscCircle(double width, double  height, Pane container, mainController mc, String name)
    {
    ...
    public void SelectionOnMouseDraggedEventHandler(MouseEvent ev) {
        //Variables i càlculs per al desplaçament de l'objecte
            ...
            double newRadius = origRadius + offsetX;
            ...
            {
                SetShapeDimensions(selection, ev, newRadius, 0, newTranslateX, newTranslateY, offsetX, offsetY);
            }
    ...
    public boolean SelectionArea(Shape sp, MouseEvent ev) {
        ...
        boolean costatDret = (ev.getX() >= (rt.getCenterX() + rt.getRadius() - 5) && ev.getX() <= rt.getCenterX() + rt.getRadius());
        boolean costatEsquerre = (ev.getX() >= rt.getCenterX() + rt.getRadius() && ev.getX() <= rt.getCenterX() + rt.getRadius() + 5);
        boolean costatSuperior = (ev.getY() >= rt.getCenterY() + rt.getRadius() && ev.getY() <= rt.getCenterY() + rt.getRadius() + 5);
        boolean costatInferior = (ev.getY() >= (rt.getCenterY() + rt.getRadius()- 5) && ev.getY() <= rt.getCenterY() + rt.getRadius());
    ...
    public void SaveShapeDimensions(Shape sp) {
        ...
        origRadius = rt.getRadius();
        origCenterX = rt.getCenterX();
        origCenterY = rt.getCenterY();
    }
    ...
    public void SetShapeDimensions(Shape sp, MouseEvent ev, double width, double height, double translateX, double translateY, double offsetX, double offsetY) {
        ...
        else if(selectedEditionEdge == asNE && isDragEditting && !isTranslating)
        {
           ...
           rt.setRadius(newHeight);
           shape.setRadius(newHeight);
    ...
    public void DrawSelectionArea(double width, double height) {
        selection = new Circle(70, 70, width); //Color transparent
